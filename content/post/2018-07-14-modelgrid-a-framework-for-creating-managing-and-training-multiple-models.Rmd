...
`modelgrid` is a new package of mine, which has just made its way on to CRAN. 
`modelgrid` offers a minimalistic but very flexible framework to create, manage 
and train a portfolio of caret models. Note, you should already be fairly familiar
with the caret package before giving `modelgrid` a spin.
...
by using the `modelgrid` package (yes, I am already familiar with the `caretEnsemble`
