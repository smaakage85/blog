...
In this post I will demonstrate, how my new R package `customsteps` can be used to generate [`recipes`](https://cran.r-project.org/web/packages/recipes/recipes.pdf) steps, that apply custom transformations to a data set. 
...
<!-- other data sets. The resulting design matrices can then -->
...
which is a quite common transformation.
...
First, we have to write a function, that estimates the relevant statistical 
...
standard deviation $s_\mathbf{x}$ to be learned from an initial data set. 
...
Let us see the function in action. I will apply it on a subset of the famous
`mtcars` data set.

```{r, message = FALSE, warning = FALSE}
library(dplyr)

# subsets 'mtcars' into two data sets.
...
Second, we have to specify a `bake` helper function, that defines how to apply
the transformation to new data using the parameters estimated from the intial 
data set. 
...
any parameters learned on the initial data set.

I will define the function `center_scale`, that will serve as my `bake` helper 
function. that will center and scale variables of a new data set.
...
center_scale <- function(x, prep_output, alpha, beta) {
  ...
  # the variables are centered around 'alpha'.
  # the variables are scaled to have a standard deviation of 'beta'.
  map2(.x = new_data,
       .y = prep_output,
...
Let us see the function in action. My first check will be to apply the 
transformation to the initial data set, that was used for estimation of
...
out of the box. In fact, we will do just that and create a recipe step 
from our new functions `compute_means_sd` and `center_scale`.

First we will create the specification of the recipe step by invoking
...
That is all there is to it! Easy.

By setting 'bake_options' to "replace", the 
selected terms will be replaced with the transformed variables, when the
recipe is baked.

I will just check, that the recipe works as expected. First we will `prep`
the recipe.
...
We will go right ahead and bake the new recipe.
